...
coloredlogs.install(level="INFO", fmt="[%(asctime)s] %(message)s", datefmt="%D:%M:%Y")
...
class Athena:
    ...
    def Tweet(self, date: str):
        ...
        body = f"#Fortnite Item Shop | {date}"

        if self.supportACreator is not None:
            body = f"{body}\n\nUse code: {self.supportACreator} in the item shop to support me! #ad"
